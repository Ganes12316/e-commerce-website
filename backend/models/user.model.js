...
userSchema.pre("save", async function () {  // ← Removed 'next' parameter
  ...
  try {
    ...
    console.log("User model: Password hashed for user:", this._id);
    // ← Removed next() call - async functions don't need it
  } catch (error) {
    console.log("User model: Error hashing password:", error.message);
    throw error;  // ← Changed from next(error) to throw error
  }
});
